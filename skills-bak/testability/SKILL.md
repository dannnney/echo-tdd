---
name: testability
description: 根据用户需求和代码库，动态生成可观测性方案。通过智能推断开发环境、观测通道、认证方式等维度，梳理清楚"系统能通过什么手段被观测和验证"。这是 Vibe Coding 自反馈机制的第一步。当用户需要了解如何验证代码、制定测试方向、或准备测试环境时使用。
---

# Testability — 可观测性方案生成器
...
5. **Phase 4: 方案生成** — 生成可观测性方案 markdown，用户审阅
...
digraph testability {
    ...
    "输出可观测性方案.md" [shape=doublecircle];
...
基于完整的六大维度信息，使用 `output-template.md` 中的模板生成可观测性方案。
...
1. 方案保存为 markdown 文件，位置由用户决定（默认 `docs/observability.md`）
...
生成方案后交由用户审阅。这是一次**可观测性对齐**——确认"我们能观测到什么、通过什么通道观测、有什么约束"。如果用户对方案有异议，调整后重新生成。

### 下一步引导

方案审阅通过后，向用户提问下一步方向：

- **环境确认 + 脚手架**（推荐）— 先验证环境、搭建测试基础设施，再生成具体用例。运行 `/testability-scaffold @<方案文档路径>` 进入阶段二
