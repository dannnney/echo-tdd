...
description: 基于阶段一的测试策略文档，验证环境可用性并生成测试脚手架代码。逐项探测前置条件和可观测性通道，生成认证 helper、数据工厂、通道客户端等测试基础设施，最后用 smoke test 验证一切就绪。当用户完成测试策略制定、准备搭建测试环境时使用。
---

# Testability Scaffold — 环境确认 + 测试脚手架生成器

基于阶段一的测试策略提案，**验证环境 → 生成脚手架 → smoke test 跑通**。

输入是一份已审阅通过的策略文档（阶段一产出），输出是可运行的测试基础设施代码。

<HARD-GATE>
必须先读取并理解策略文档，才能开始任何探测。不要凭空猜测环境情况——策略文档中已经包含了所有需要验证的前置条件。
</HARD-GATE>
...
1. **Phase 0: 策略解析** — 读取策略文档，提取探测清单和关键信息
...
digraph scaffold {
    ...
    "用户触发 /testability-scaffold" -> "Phase 0: 策略解析";
    "Phase 0: 策略解析" -> "Phase 1: 环境探测";
