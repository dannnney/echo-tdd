---
name: testability-cases
description: 基于阶段一的测试策略和阶段二的环境确认结果，生成个性化的测试用例文档和测试数据蓝图。通过规格推导、环境采样、属性建模、用户旅程等多种方法系统化生成高覆盖度的用例。产出是纯文本用例，不是测试代码。当用户完成测试脚手架搭建、准备设计具体测试用例时使用。
---

# Testability Cases — 测试用例 + 数据蓝图生成器

基于前两阶段的产出，生成**个性化的测试用例文档 + 测试数据蓝图**。

输入是阶段一的策略文档 + 阶段二的探测报告/脚手架代码。输出是可供后续阶段转化为代码的完整用例文本。
...
digraph cases {
    ...
    "用户触发 /testability-cases" [shape=doublecircle];
    ...
    "通过？" [shape=diamond];
    "输出：用例文档 + 数据蓝图" [shape=doublecircle];

    "用户触发 /testability-cases" -> "Phase 0: 输入解析";
